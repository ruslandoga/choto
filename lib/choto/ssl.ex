...
defmodule Choto.SSL do
  ...
  defp get_cacertfile(path) do
    if Application.get_env(:choto, :persistent_term) do
      case :persistent_term.get({:choto, {:cacertfile, path}}, :error) do
        {:ok, cacerts} ->
          cacerts

        :error ->
          cacerts = decode_cacertfile(path)
          :persistent_term.put({:choto, {:cacertfile, path}}, {:ok, cacerts})
          cacerts
      end
    else
